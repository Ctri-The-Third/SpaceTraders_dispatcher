...
class BehaviourConductor:
    ...
    def hourly_update(self, system: "ConductorSystem"):
        ...
        for ship in self.all_ships:
            set_behaviour(ship.name, "DISABLED FOR EMERGENCY", {}, self.connection)
        ...
        possible_haulers = math.floor(self.st.view_my_self().credits / 1000000)
        best_trader = system.haulers.pop(0)
        set_behaviour(
            best_trader.name, bhvr.BHVR_TRADE_BEST_INTRASOLAR, {}, self.connection
        )
        for i in range(possible_haulers):
            if system.haulers:
                hauler = system.haulers.pop(0)
                set_behaviour(hauler.name, bhvr.BHVR_CHAIN_TRADE, {}, self.connection)
